...
const DEFAULT_SHOPS: readonly PickShop[] = [
  ...
  },
  {
    ...
    folders: ["좋아요", "가고 싶은 곳", "선물 사기 좋은 곳"],
    memo: "주말 오후에 들러보기",
  },
  {
    id: "gacha",
  ...
  },
  {
    ...
    region: "망원",
    category: "문구샵",
    imageUrl: "/images/brand/mascot.webp",
    folders: ["좋아요", "가고 싶은 곳"],
  },
  {
    id: "mood",
    ...
    region: "성수",
    category: "소품샵",
    imageUrl: "/images/profiles/user_default.webp",
    folders: ["좋아요"],
  },
] as const;
...
export function PicksScreen({
  ...
  const filteredShops = initialShops.filter((shop) =>
    shop.folders.includes(activeFolder),
  );
  const folderCount = (folder: string) =>
    initialShops.filter((shop) => shop.folders.includes(folder)).length;
  ...
  return (
    <section className="min-h-full bg-white px-5 pb-24 pt-16">
      <header className="flex items-end justify-between">
        ...
        <span className="rounded-full bg-green-100 px-3 py-1.5 text-14 font-bold text-green-700">
          🍀 {initialShops.length}개
        </span>
      </header>
      ...
      >
        {folders.map((folder) => (
          <button
            ...
            className={[
              "min-h-9 shrink-0 rounded-full border px-3 text-12",
              activeFolder === folder
                ? "border-black-900 bg-black-900 font-bold text-white"
      ...
      {filteredShops.length === 0 ? (
        <StatePanel title="아직 이 폴더에 담긴 상점이 없어요" />
      ) : (
        <div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-5">
          {filteredShops.map((shop) => (
            <article key={shop.id} className="min-w-0">
              <Link
                href={`/shops/${shop.id}`}
              ...
              >
                ...
                <h2 className="mt-3 truncate text-15 font-medium">
                  {shop.name}
                </h2>
                <div className="mt-1 flex items-center gap-2 text-12">
                  <span className="truncate text-black-500">{shop.region}</span>
                  <span className="rounded-full bg-green-100 px-2 py-0.5 text-black-600">
                    {shop.category}
                  </span>
                </div>
                {shop.memo && activeFolder !== "좋아요" ? (
                  <p className="mt-2 line-clamp-2 text-12 text-black-500">
                    메모 · {shop.memo}
                  </p>
                ) : null}
              </Link>
      ...
      <div className="fixed bottom-20 right-[max(20px,calc((100vw-480px)/2+20px))] z-20 flex flex-col-reverse items-end gap-3">
        <button
          ...
          className="grid size-13 place-items-center rounded-full border border-black-100 bg-white text-32 font-light text-green-700 shadow-sm"
        >
          {menuOpen ? "×" : "+"}
        </button>
        {menuOpen ? (
          <div
            ...
            className="w-52 rounded-2xl bg-white p-2 text-13 shadow-[0_4px_20px_rgba(0,0,0,0.12)]"
          >
            <button
              ref={menuFirstItemRef}
              type="button"
              onClick={() => {
                onCreateCourse(activeFolder);
                setMenuOpen(false);
              }}
              className="min-h-11 w-full rounded-xl px-3 text-left hover:bg-black-100"
            >
              이 폴더 속 샵으로 코스 만들기
            </button>
            <button
...
function PicksSkeleton() {
  return (
    <section className="p-5 pt-16">
      <Skeleton label="내 픽을 불러오는 중" className="h-14 w-36 rounded-xl" />
