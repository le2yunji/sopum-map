// (main)/shops/[shopId]/_components/ShopDetailScreen.tsx
    ...
    <main className="min-h-dvh bg-black-100 pb-10">
      ...
      <div className="-mt-5 relative rounded-t-[20px] bg-white px-5 pt-5 pb-6">
        ...
        <div className="mt-5 grid grid-cols-2 gap-2">
          <Button variant="outline" onClick={() => setSheet("pick")}>
            내 픽에 추가
          </Button>
          <Link
            href={`/shops/${shop.id}/reviews/new`}
            className="flex min-h-11 items-center justify-center rounded-xl border border-pink-300/30 text-14 font-semibold"
          >
            방문 후기 작성
          </Link>
        </div>
        {pickMessage ? (
          <p role="status" className="mt-3 text-center text-13 text-green-800">
            {pickMessage}
          </p>
        ) : null}
      </div>
      ...
      <section aria-label="상점 위치" className="mt-2 bg-white px-5 py-6">
        ...
        <div className="relative mt-3 aspect-[345/176] overflow-hidden rounded-xl bg-green-100">
          <Image
            fill
            src={shop.mapImageUrl ?? "/images/shops/shop_example.png"}
            alt={`${shop.name} 위치 지도`}
      ...
      <section className="mt-2 bg-white px-5 py-6">
        <h2 className="text-16 font-semibold">방문 정보</h2>
        <dl className="mt-4 space-y-3 text-13">
          <div className="flex">
            <dt className="w-20 text-black-500">영업시간</dt>
            <dd className="font-semibold">{shop.hours}</dd>
          </div>
          {shop.closedDay ? (
            <div className="flex">
              <dt className="w-20 text-black-500">휴무일</dt>
      ...
      <section className="mt-2 bg-white px-5 py-6">
        <div className="flex items-center justify-between">
          ...
          <span className="flex items-center gap-1 text-12 text-green-700">
            <HeartIcon className="text-red-600" />
            {shop.likeCount}
          </span>
        </div>
        {shop.reviews.length === 0 ? (
          <StatePanel title="아직 등록된 방문 후기가 없어요" />
        ) : (
          <div className="mt-5 space-y-7">
            {shop.reviews.map((review) => (
              <article key={review.id}>
                ...
                <p className="mt-3 text-13 leading-5 text-black-800">
                  {review.content}
                </p>
              </article>
            ))}
      ...
      <BottomSheet
        open={sheet === "pick"}
        onOpenChange={(open) => setSheet(open ? "pick" : null)}
      ...
      <BottomSheet
        open={sheet === "report"}
        onOpenChange={(open) => (open ? setSheet("report") : closeReport())}
        ariaLabel="정보 수정 제보"
      >
        {reported ? (
          <div
          ...
          >
            ...
            <Button fullWidth className="mt-6" onClick={closeReport}>
              닫기
            </Button>
          </div>
        ) : (
