  ...
  return (
    <main className="min-h-dvh bg-black-100 pb-10">
      <section className="relative h-[318px] bg-black-300" aria-label="상점 이미지">
        ...
        <div className="absolute inset-x-4 top-5 flex justify-between">
          ...
          <div className="flex gap-2">
            <button type="button" aria-pressed={liked} aria-label={liked ? "좋아요 취소" : "좋아요 추가"} onClick={() => setLiked(!liked)} className={["grid size-11 place-items-center rounded-full focus-visible:outline-2 focus-visible:outline-green-700", liked ? "bg-pink-100" : "bg-white/90"].join(" ")}><Image data-testid="heart-icon" src="/images/icons/heart.svg" alt="" width={26} height={23} /></button>
            <button type="button" aria-label="정보 수정 제보" onClick={() => setSheet("report")} className="grid size-11 place-items-center rounded-full bg-white/90 focus-visible:outline-2 focus-visible:outline-green-700"><Image data-testid="pen-icon" src="/images/icons/pen.svg" alt="" width={18} height={18} /></button>
          </div>
        </div>
      ...
      <div className="-mt-5 relative rounded-t-[20px] bg-white px-5 pt-5 pb-6">
        <div className="flex items-start justify-between gap-3"><div><h1 className="text-20 font-semibold">{shop.name}</h1><p className="mt-1 flex items-center gap-1.5 text-12 text-black-500"><Image data-testid="location-icon" src="/images/icons/location.svg" alt="" width={9} height={10} />{shop.distance}</p></div><Badge><Image data-testid="store-icon" src="/images/icons/store.svg" alt="" width={15} height={14} className="mr-1" />{shop.category}</Badge></div>
