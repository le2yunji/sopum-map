...
- `/picks`와 `/me` 링크는 하단 내비게이션에 있지만 `dev`에는 대응 페이지가 없다. `/picks` UI는 PR #97에서 검토 중이다.
- 상점 상세 UI는 PR #95, 방문 후기 작성 UI는 PR #96에서 검토 중이다.
- 코스 상세 UI는 `feat/42/course-detail-ui`에서 구현 중이다.
- 로그인 버튼은 UI만 있고 실제 소셜 인증 흐름은 없다.
- 좋아요, 방문 기록, 보관 폴더, 코스 관리 API 연결이 없다.
- 화면별 실제 네트워크 로딩·실패 처리와 섹션 단위 오류 격리는 API 연결 때 추가해야 한다.

### 진행 중인 UI 작업

- 공용 Modal은 구현하지 않는다. 선택과 추가 입력은 `BottomSheet`, 결과와 확인은 페이지 안 상태 UI를 사용한다.
- PR #95 상점 상세 UI, PR #96 방문 후기 작성 UI, PR #97 내 픽 UI가 `dev` 병합을 기다리고 있다.
- 남은 순수 UI 이슈는 #42 코스 상세와 #45 마이페이지다. #42는 `feat/42/course-detail-ui`에서 구현 중이다.
- 과거 브랜치의 `prunable` 워크트리 메타데이터는 임의로 정리하지 않는다. 새 작업은 최신 `dev`를 기준으로 한다.
