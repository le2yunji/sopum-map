...
- 코스 상세 UI는 PR #98에서 검토 중이며 마이페이지 UI는 `feat/45/mypage-ui`에서 구현 중이다.
...
- PR #95 상점 상세, #96 방문 후기 작성, #97 내 픽, #98 코스 상세 UI가 `dev` 병합을 기다리고 있다.
- 마지막 순수 UI 이슈 #45 마이페이지는 `feat/45/mypage-ui`에서 구현 중이다.
