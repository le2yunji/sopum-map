...
- Tailwind CSS 4
- Storybook 10.5.4
- Vitest 4.1.10과 Playwright 1.62.0
...
- `Badge`, `BottomSheet`, `Button`, `SearchInput`, `FilterChipGroup`, `Skeleton`, `Textarea` 공용 컴포넌트
...
- 상점 상세 페이지는 Figma 기반 fixture 화면과 좋아요·내 픽·정보 수정 제보 로컬 흐름을 제공한다. 제보 완료는 Modal 대신 같은 BottomSheet 안의 완료 상태로 표시한다.
- 코스 상세와 방문 후기 작성 페이지는 대응 라우트가 없다.
...
- 공용 팝업 컴포넌트는 구현하지 않는다. 선택·추가 입력 흐름은 `BottomSheet`, 단순 안내와 작업 결과는 페이지 안의 상태 UI를 사용한다.
- 이슈 #33 내 픽, #39 후기 작성, #42 코스 상세, #45 마이페이지 UI가 열려 있다. #36 상점 상세 UI는 `feat/36/shop-detail-ui-v2`에서 구현 중이다.
- `feat/36/shop-detail-ui`와 `feat/39/visit-log-ui`에는 닫힌 PR의 미병합 구현이 남아 있다. 그대로 병합하지 말고 최신 `dev`와 현재 UI 결정을 기준으로 필요한 코드만 검토한다.
- 이슈 #33, #42, #45는 최신 `dev` 기준 구현이 없다.
- 과거 작업용 워크트리 경로는 모두 사라졌고 Git에는 `prunable` 메타데이터만 남아 있다. 임의로 정리하지 않는다.
...
2. `frontend/AGENTS.md`와 이슈의 완료 조건을 읽는다. `docs/superpowers/`는 현재 `dev`에 없으므로 과거 브랜치 문서를 최신 기준으로 간주하지 않는다.
...
- 남아 있는 여러 `prunable` 워크트리 메타데이터와 과거 브랜치는 사용자 확인 뒤 정리해야 하며 임의로 삭제하지 않는다.
